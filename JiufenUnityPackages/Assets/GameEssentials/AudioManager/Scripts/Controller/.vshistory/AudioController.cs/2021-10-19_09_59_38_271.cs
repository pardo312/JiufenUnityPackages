...
namespace Jiufen.Audio
{
    public partial class AudioController : MonoBehaviour
    {
        ...
        private void AddJob(AudioJob _audioJob)
        {
            ...
            //Add Job
IEnumator jobRunner = RunAudioJob(_audioJob)
        }
        ...
        private void RemoveConflictingJobs(AudioType audioType)
        {
            ...
            {
                AudioTrack currentTrack = (AudioTrack)_audioTable[job.Key];
                AudioTrack newTrack = (AudioTrack)_audioTable[audioType];
                if (currentTrack._audioSource == newTrack._audioSource)
                {
                    LogError($"You have the same audio source for different audioTypes. Please check audioType [{job.Key}] and [{audioType}] ");
