        ...
        private void ConfigureAudioTable()
        {
            foreach (AudioTrack tracks in _audioTracks)
            {
                foreach (AudioObject audioObj in tracks._audioObject)
                {
                    ...
                    {
                        LogError($"AudioTable is registering an already registered audio type: {audioObj._audioType.ToString()}");
                    }
                    else
                    {
                        _audioTable.Add(audioObj._audioType, tracks);
                        Log($"Audio type {audioObj._audioType.ToString()} has been registered in the audioTable");
                    }
                }
            }
